...
# FIX System Python incompatibilities for 3.12
RUN apt-get remove -y python3-lxml python3-psutil python3-tz python3-pytz python3-pyinotify python3-cffi python3-cryptography python3-openssl python3-urllib3 || true && \
    pip3 uninstall -y lxml lxml-html-clean psutil pytz cffi cryptography pyOpenSSL urllib3 && \
    pip3 install --no-cache-dir --ignore-installed lxml lxml-html-clean psutil pytz cffi cryptography pyOpenSSL urllib3==1.26.18
...
# Copy entrypoint and ensure extra-addons directory exists
COPY ./config/entrypoint.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh \
    && mkdir -p /workspace/extra-addons \
    && chown -R odoo:odoo /workspace/extra-addons

# Final setup — root is kept so the entrypoint can fix volume permissions
